...
const burger = {
  ...
  indirim: function (discount) {
    if (discount === "öğretmen" || discount === "öğrenci") {
      return this.fiyat * 0.75;
    } else if (discount === "diğer") {
...
for (let i = 0; i < degerlendirmeler.length; i++) {
  if (degerlendirmeler[i].isim === "Ahmet") {
    console.log(degerlendirmeler[i].geribildirim, "Görev3");
  }
}
...
function DegerledirmeEkle(reviews, name, score, feedback) {
  const newReview = {
    isim: name,
    puan: score,
...
// console.log(DegerledirmeEkle(degerlendirmeler, "Hurşut", 2, "Kötü!"));
...
function AnahtardanDegerlendirmeAl(reviews, key) {
  const review = reviews[key];
  return `${review.isim} isimli kişi ${review.puan} puan verdi ve şunları yazdı: ${review.geribildirim}`;
}
...
function SonDegerlendirmeyiAl(reviews) {
  const lastReview = reviews[reviews.length - 1];
  return `${lastReview.isim} isimli kişi ${lastReview.puan} puan verdi ve şunları yazdı: ${lastReview.geribildirim}`;
}
...
function PuanaGoreDegerlendirmeAl(reviews, score) {
  const filteredReviews = [];
  for (let i = 0; i < reviews.length; i++) {
    if (Math.floor(reviews[i].puan) === score) {
  ...
  // Alternative solution with filter method and explicit return
  // const filteredReviews = reviews.filter((review) => {
  //   return Math.floor(review.puan) === score;
  // });
  // return filteredReviews;

  // Alternative solution with filter method and implicit return
  // return reviews.filter((review) => Math.floor(review.puan) === score);
}
...
function UzunDegerlendirmeleriAl(reviews) {
  const filteredReviews = [];
  for (let i = 0; i < reviews.length; i++) {
    if (reviews[i].geribildirim.split(" ").length >= 15) {
      filteredReviews.push(reviews[i]);
    }
  }
  ...
  // const filteredReviews = reviews.filter((review) => {
  //   return review.geribildirim.split(" ").length >= 15;
  // });
